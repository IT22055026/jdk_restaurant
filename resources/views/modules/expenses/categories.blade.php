...
@section('content')
    <div>
        <div class="mb-6">
            <a href="{{ route('expenses.index') }}" class="flex items-center justify-center w-9 h-9 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-600 hover:text-gray-900 transition-colors mb-2" title="Back">
            ...
            @section('breadcrumb')
                <nav class="text-sm text-gray-600" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-2">
                        ...
                        <li><a href="{{ route('expenses.index') }}" class="text-gray-500 hover:text-gray-700">Purchases & Expenses</a></li>
                        <li class="text-gray-300">/</li>
                        <li class="text-gray-900 font-semibold">Manage Categories</li>
                    </ol>
                </nav>
            <h1 class="text-4xl font-bold text-gray-900 mt-2">Manage Expense Categories</h1>
            @endsection
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-6">
            ...
            <form action="{{ route('expense-categories.store') }}" method="POST" class="flex flex-wrap items-end gap-3">
                @csrf
                <input type="hidden" name="type" value="main">
                <div class="flex-1 min-w-[200px]">
                    <label class="block text-xs font-medium text-gray-500 mb-1">Category Name</label>
                    <input type="text" name="name" required placeholder="e.g. Insurance"
                ...
                <div class="flex-1 min-w-[240px]">
                    ...
                    <input type="text" name="description" placeholder="What this category covers"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                    <div class="p-5 bg-gray-50 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-400">
                                    {{ $main->children->count() }} sub-categories
                                    @if($main->expenses_count) &middot; {{ $main->expenses_count }} expense(s) recorded @endif
                                </p>
                            </div>
                            @if(!$main->is_active)
                        ...
                        <div class="flex items-center gap-4">
                            <form action="{{ route('expense-categories.update', $main) }}" method="POST" class="flex flex-wrap items-center gap-2">
                                @csrf
                                @method('PUT')
                                ...
                                <input type="text" name="description" value="{{ $main->description }}" placeholder="Description"
                                    class="text-sm px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent w-48">
                            ...
                            <form action="{{ route('expense-categories.destroy', $main) }}" method="POST" onsubmit="return confirm('Delete this main category?')">
                                @csrf
                                @method('DELETE')
                    ...
                    <div class="p-5">
                            ...
                            <div class="divide-y divide-gray-100 mb-4">
                                @foreach($main->children as $sub)
                                    <div class="py-3 flex flex-wrap items-center justify-between gap-3">
                                        ...
                                        <div class="flex items-center gap-4">
                                            <form action="{{ route('expense-categories.update', $sub) }}" method="POST" class="flex flex-wrap items-center gap-2">
                                                @csrf
                                                @method('PUT')
                                                <input type="text" name="name" value="{{ $sub->name }}" required
                                                    class="text-sm px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                                <input type="text" name="description" value="{{ $sub->description }}" placeholder="Description"
                                                    class="text-sm px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent w-40">
                                                <label class="inline-flex items-center gap-1.5 text-xs text-gray-600">
                                            ...
                                            <form action="{{ route('expense-categories.destroy', $sub) }}" method="POST" onsubmit="return confirm('Delete this sub-category?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-800 text-sm font-semibold">
                        ...
                        <form action="{{ route('expense-categories.store') }}" method="POST" class="flex flex-wrap items-end gap-3 pt-3 border-t border-gray-100">
                            @csrf
                            ...
                            <div class="flex-1 min-w-[200px]">
                                ...
                                <input type="text" name="name" required placeholder="e.g. Bonus"
