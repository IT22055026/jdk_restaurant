...
@section('content')
    @php
        $selectedIngredientIds = old('ingredient_ids', $offer->ingredients->pluck('id')->all());
        $selectedIngredientQuantities = old('ingredient_quantities', old('quantities', $offer->ingredients->mapWithKeys(fn($i) => [$i->id => $i->pivot->quantity])->all()));

        $selectedProductIds = old('product_ids', $offer->products->pluck('id')->all());
        $selectedProductQuantities = old('product_quantities', $offer->products->mapWithKeys(fn($p) => [$p->id => $p->pivot->quantity])->all());

        // Choice groups data for initial JS hydration
        $initialChoiceGroups = $offer->choiceGroups->map(function($cg) {
            return [
                'name' => $cg->name,
                'category_id' => $cg->category_id,
                'choice_qty' => $cg->choice_qty,
                'product_ids' => $cg->products->pluck('id')->all(),
            ];
        })->values()->all();

        // Fallback to legacy flavours if no choiceGroups
        if (empty($initialChoiceGroups) && $offer->flavours->isNotEmpty()) {
            $initialChoiceGroups = [[
                'name' => 'Flavours',
                'category_id' => null,
                'choice_qty' => $offer->flavour_qty ?? 1,
                'product_ids' => $offer->flavours->pluck('id')->all(),
            ]];
        }
    @endphp
    <div>
                ...
                @section('breadcrumb')
                    <nav class="text-sm text-gray-600" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-2">
                            ...
                            <li class="text-gray-300">/</li>
                            <li class="text-gray-900 font-semibold">Edit Offer</li>
                        </ol>
                    </nav>
                <h1 class="text-4xl font-bold text-gray-900">Edit Offer</h1>
                @endsection
            </div>
            <p class="text-gray-600 mt-2">Update "{{ $offer->name }}" inclusions and choices</p>
        </div>

        <form action="{{ route('offers.update', $offer) }}" method="POST" enctype="multipart/form-data" class="space-y-8" id="offerForm">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="price" id="price" value="{{ old('price', $offer->price) }}" step="0.01" min="0" required
                ...
                <div class="mt-6">
                    ...
                    <textarea name="description" id="description" rows="2"
                        ...
                        placeholder="Short description shown in POS">{{ old('description', $offer->description) }}</textarea>
                    @error('description')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 items-center">
                    <div>
                        ...
                        <img id="image-preview"
                            src="{{ $offer->image ? (Str::startsWith($offer->image, ['http://', 'https://']) ? $offer->image : asset("storage/{$offer->image}")) : '#' }}"
                            alt="{{ $offer->name }}"
                            class="mt-2 w-24 h-24 object-cover rounded-lg border border-gray-200 {{ $offer->image ? '' : 'hidden' }}">
                    </div>
                                    ...
                                    <tr data-name="{{ strtolower($ingredient->name) }}">
                                        <td class="px-4 py-2">
                                            <input type="checkbox" name="ingredient_ids[]" value="{{ $ingredient->id }}"
                                                {{ in_array($ingredient->id, $selectedIngredientIds) ? 'checked' : '' }}
                                                class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                        </td>
                                        ...
                                        <td class="px-4 py-2">
                                            <div class="relative">
                                                <input type="number" name="ingredient_quantities[{{ $ingredient->id }}]" step="0.001" min="0.001"
                                                    value="{{ $selectedIngredientQuantities[$ingredient->id] ?? 1 }}"
...
<script>
    ...
    const initialChoiceGroups = @json($initialChoiceGroups);
    ...
    // Hydrate existing choice groups on page load
    document.addEventListener('DOMContentLoaded', function() {
        if (initialChoiceGroups && initialChoiceGroups.length > 0) {
            initialChoiceGroups.forEach(g => addChoiceGroup(g));
        }
    });
</script>
@endsection
