...
@section('content')
    @php
        $selectedIds = old('ingredient_ids', $offer->ingredients->pluck('id')->all());
        $selectedQuantities = old('quantities', $offer->ingredients->mapWithKeys(fn($i) => [$i->id => $i->pivot->quantity])->all());
        $selectedFlavourIds = old('flavour_ids', $offer->flavours->pluck('id')->all());
    @endphp
    <div>
        <div class="mb-8">
            <div>
            ...
            <p class="text-gray-600 mt-2">Update "{{ $offer->name }}"</p>
        </div>

        <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-8 w-full">
            <form action="{{ route('offers.update', $offer) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <img id="image-preview"
                        ...
                        alt="{{ $offer->name }}"
                        class="mt-2 w-28 h-28 object-cover rounded-lg border border-gray-200 {{ $offer->image ? '' : 'hidden' }}">
                </div>
                                    ...
                                    <tr>
                                        <td class="px-4 py-2">
                                            <input type="checkbox" name="ingredient_ids[]" value="{{ $ingredient->id }}"
                                                {{ in_array($ingredient->id, $selectedIds) ? 'checked' : '' }}
                                                class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                        </td>
                                        ...
                                        <td class="px-4 py-2">
                                            <div class="relative">
                                                <input type="number" name="quantities[{{ $ingredient->id }}]" step="0.001" min="0.001"
                                                    value="{{ $selectedQuantities[$ingredient->id] ?? 1 }}"
                                    ...
                                    <tr data-name="{{ strtolower($product->name) }}">
                                        <td class="px-4 py-2 w-10">
                                            <input type="checkbox" name="flavour_ids[]" value="{{ $product->id }}"
                                                {{ in_array($product->id, $selectedFlavourIds) ? 'checked' : '' }}
                                                class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                        </td>
