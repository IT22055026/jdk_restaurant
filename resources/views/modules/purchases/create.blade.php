...
@section('content')
    <div>
        <div class="mb-8">
            ...
            <p class="text-gray-600 mt-2">Record a supplier purchase — pick a main category, then the specific item</p>
        </div>

        <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-8 w-full">
            <form action="{{ route('purchases.store') }}" method="POST" class="space-y-0">
                @csrf
