                ...
                @section('breadcrumb')
                    <nav class="text-sm text-gray-600" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-2">
                            ...
                            <li class="text-gray-900 font-semibold">Record Wastage</li>
                        </ol>
                    </nav>
                <h1 class="text-4xl font-bold text-gray-900">Record Wastage</h1>
                @endsection
            </div>
            <p class="text-gray-600 mt-2">Record product wastage or loss</p>
        </div>

        <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-8 w-full">
            <form action="{{ route('wastages.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', now()->format('Y-m-d')) }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent {{ $errors->has('date') ? 'border-red-600' : '' }}">
                    @error('date')
                        <p class="text-red-600 text-sm mt-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        ...
                        placeholder="Additional notes about the wastage">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="text-red-600 text-sm mt-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex gap-4 pt-4">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-semibold transition-colors">
                        <i class="fas fa-save mr-2"></i>Record Wastage
                    </button>
